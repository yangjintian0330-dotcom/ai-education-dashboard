...
assert.match(source, /\.metric-definitions\{[^}]*grid-template-columns:repeat\(6,1fr\)/);
assert.equal((metrics.match(/class="metric"/g) ?? []).length, 5);
for (const label of ['应用学校数', '赋能老师数', '辅助工作数', '执行任务数', '资源沉淀数', '资源复用数']) {
...
assert.match(source, /<small>学情分析 · 知识库 · 题库等 AI 调用<\/small>/);
...
assert.match(source, /animateNumber\('reuse'/);
